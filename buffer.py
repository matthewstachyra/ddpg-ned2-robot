...
class ReplayBuffer:
    def __init__(self, max_size, input_shape, n_actions):
        ...
        self.state_memory = np.zeros((self.mem_size, 6))
        self.new_state_memory = np.zeros((self.mem_size, 6))
        self.action_memory = np.zeros((self.mem_size, 6))
        self.reward_memory = np.zeros(self.mem_size)
        self.terminal_memory = np.zeros(self.mem_size, dtype=np.float32)

    def store(self, state, action, reward, sprime, done):
        index = self.count % self.mem_size
        self.state_memory[index] = state
        self.action_memory[index] = action
        self.reward_memory[index] = reward
        self.new_state_memory[index] = sprime
